...
const CartItemRow = ({
  ...
  const artwork = featuredArtworks.find((a) => a.id === item.artworkId);
  ...
  return (
    ...
    >
      <img src={imageSrc} alt={artwork.title} className="h-20 w-20 rounded-lg object-cover" />
      <div className="flex-1 flex flex-col justify-between">
        <div>
          <h3 className="font-display text-sm font-semibold text-foreground line-clamp-1">{artwork.title}</h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onRemove}
              className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-destructive transition-colors ml-1"
            >
...
const Cart = () => {
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([
    ...
    { artworkId: "3", quantity: 1 },
  ]);

  const updateQuantity = (id: string, qty: number) => {
    setCartItems((items) => items.map((i) => (i.artworkId === id ? { ...i, quantity: qty } : i)));
  };

  const removeItem = (id: string) => {
    setCartItems((items) => items.filter((i) => i.artworkId !== id));
    ...
    <div className="min-h-screen bg-background pb-48">
      ...
      <main className="mx-auto max-w-lg px-4 pt-4 space-y-3">
        <AnimatePresence mode="popLayout">
          {cartItems.map((item) => (
            <CartItemRow
              key={item.artworkId}
              item={item}
              onUpdate={(qty) => updateQuantity(item.artworkId, qty)}
        ...
        <div className="flex items-center gap-2 rounded-xl border border-border bg-card p-3">
          ...
          />
          <button className="rounded-lg bg-gradient-gold px-3 py-1.5 text-xs font-semibold text-foreground">
            Apply
          </button>
        </div>
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 border-t border-border bg-background/95 backdrop-blur-lg p-4">
        <div className="mx-auto max-w-lg">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-burgundy py-3.5 text-sm font-semibold text-primary-foreground shadow-gold transition-transform hover:scale-[1.01] active:scale-[0.99]"
          >
            Proceed to Checkout
